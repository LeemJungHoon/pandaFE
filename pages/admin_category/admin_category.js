...
window.addEventListener('load', async () => {
  ...
  try {
    const response = await fetch('http://kdt-sw-6-team10.elicecoding.com/api/categories');
    ...
    categories.forEach((category, index) => {
      ...
      newMainCategory.addEventListener('click', async () => {
        ...
        activeMainCategory = newMainCategory;
        try {
          const subCategoryResponse = await fetch(`http://kdt-sw-6-team10.elicecoding.com/api/subCategories/${category.name}`, {
            headers: {
  ...
  addSubBtn.addEventListener('click', () => {
    ...
    input.addEventListener('keydown', async (event) => {
      if (event.key === 'Enter') {
        if (input.value.trim() !== '') {
          let currentMainCategoryName = document.querySelector('.active').innerText;
          ...
          // 카테고리 이름을 서버로 보내기
          try {
            const response = await fetch('http://kdt-sw-6-team10.elicecoding.com/api/subcategories', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
    ...
    if (activeSubCategory) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const subCategoryName = subcategories.find((subCategory) => subCategory.name === activeSubCategory.textContent);
          ...
          input.addEventListener('keydown', async (event) => {
            if (event.key === 'Enter') {
              ...
              if (input.value.trim() !== activeSubCategory.textContent) {
                try {
                  // 서버로 수정 요청 보내기
                  const patchResponse = await fetch(`http://kdt-sw-6-team10.elicecoding.com/api/subcategories/${subCategoryName._id}`, {
                    method: 'PATCH',
                    headers: {
                      'Content-Type': 'application/json',
                    },
      ...
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          const categories = data.data;
          const categoryName = categories.find((category) => category.name === activeCategory.textContent);
          ...
          input.addEventListener('keydown', async (event) => {
            if (event.key === 'Enter') {
              ...
              if (input.value.trim() !== activeCategory.textContent) {
                try {
                  // 서버로 수정 요청 보내기
                  const patchResponse = await fetch(`http://kdt-sw-6-team10.elicecoding.com/api/categories/${categoryName._id}`, {
                    method: 'PATCH',
                    headers: {
                      'Content-Type': 'application/json',
                    },
  ...
  deleteButton.addEventListener('click', async () => {
    ...
    if (activeSubCategory) {
      const deleteConfirmed = window.confirm(`${activeSubCategory.innerText}을(를) 삭제하시겠습니까?`);
      if (deleteConfirmed) {
        try {
          // 서버로 GET 요청 보내서 name과 일치하는 카테고리의 _id 가져오기
          const response = await fetch('http://kdt-sw-6-team10.elicecoding.com/api/subcategories');
      ...
      const deleteConfirmed = window.confirm(`${activeCategory.innerText}을(를) 삭제하시겠습니까?`);
      if (deleteConfirmed) {
        try {
          // 서버로 GET 요청 보내서 name과 일치하는 카테고리의 _id 가져오기
          const response = await fetch('http://kdt-sw-6-team10.elicecoding.com/api/categories');
...
function makeCancelBtn() {
  ...
  const editCategory = document.querySelector('.category_edit_button_container');
