...
// 각각의 상품 총 가격
const totalProductPrice = document.getElementsByClassName(
  "total_product_price"
)[0];
// 우측 전체 상품 가격
const cartPopTotalPrice = document.getElementsByClassName(
  "cart_pop_total_price"
)[0];
// 선택상품삭제 버튼
...
const cartMemberLogic = async () => {
  console.log("회원로직");
  try {
    await fetch("https://panda-be.vercel.app/api/cart", {
      headers: {
      ...
      .then((dbData) => {
        ...
        cartPopTotalPrice.innerHTML = totalPrice
          .toString()
          .replace(/(\d)(?=(?:\d{3})+(?!\d))/g, "$1,");
      });
  } catch (err) {
...
const cartNonMemberLogic = () => {
  console.log("비회원로직");
  localStorage.getItem("nonMembercartProduct")
    ? 0
  ...
  if (localStorage.getItem("login-token")) {
    ...
    for (let i = 1; i <= document.querySelectorAll(".cart_item").length; i++) {
      if (cbxCartItem[i - 1].checked) {
        cartItem[i - 1] = `${
          document.querySelectorAll(".cart_item")[i - 1].id
        }`;
      }
    }
    ...
    arrProductData.forEach((item) => {
      ...
      if (!localStorage.getItem("nonMembercartProduct")) {
        localStorage.setItem(
          ...
          `${arrItem[0]} ${arrItem[1]} ${arrItem[2]} ${arrItem[3]}`
        );
      } else {
        localStorage.setItem(
          "nonMembercartProduct",
          `${localStorage.getItem("nonMembercartProduct")}/${arrItem[0]} ${
            arrItem[1]
          } ${arrItem[2]} ${arrItem[3]}`
        );
      }
...
allItemDeleteBtn.addEventListener("click", async () => {
  if (localStorage.getItem("login-token")) {
    ...
    await fetch("https://panda-be.vercel.app/api/pages/cart", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `bearer ${localStorage.getItem("login-token")}`,
...
orderBtn.addEventListener("click", async () => {
  try {
    if (localStorage.getItem("login-token")) {
      ...
      for (
        let i = 0;
        i <= document.querySelectorAll(".cart_item").length - 1;
        i++
      ) {
        cartData[i] = {
          item: `${document.querySelectorAll(".cart_item")[i].id}`,
          quantity: `${Number(document.querySelectorAll(".item_count")[i].id)}`,
        };
      }
...
cartListItem.addEventListener("click", (e) => {
  ...
  if (localStorage.getItem("login-token")) {
    ...
    if (e.target.innerHTML === "▲") {
      ...
      // 우측 총 금액 수정.
      cartPopTotalPrice.innerHTML = (
        Number(cartPopTotalPrice.innerHTML.split(",").join("")) + Number(pdp)
      )
        .toString()
        .replace(/(\d)(?=(?:\d{3})+(?!\d))/g, "$1,");
    ...
    if (e.target.innerHTML === "▲") {
      arrProductData.forEach((item) => {
        ...
        // 다시 스토리지에 넣을거임...
        // 진짜 개복잡ㅎ하다
        arrItem[0] === productName
          ? (arrItem[3] = `${Number(arrItem[3]) + 1}`)
          : 0;
  ...
  const makeCartProductHTML = `
    <div class="cart_item" id=${productItem}>
        ...
        <div class="item_content" id="${productName}">
            <div class="left">
                <img src="https://panda-be.vercel.app/${productImg}" alt="" />
            </div>
            <div class="right">
                <p class="item_price" id="${productPrice}">금액 : ${productPrice
    .toString()
    .replace(/(\d)(?=(?:\d{3})+(?!\d))/g, "$1,")} 원</p>
                ...
                <p class="total_price">총 금액 : 
                    <span class="total_product_price">${(
                      Number(productPrice) * Number(productNum)
                    )
                      .toString()
                      .replace(
                        /(\d)(?=(?:\d{3})+(?!\d))/g,
