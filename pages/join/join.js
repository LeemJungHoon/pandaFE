...
const phone_number_input_startnum = document.getElementsByClassName("phone_number_input_startnum")[0];
const phone_number_input_lastnum = document.getElementsByClassName("phone_number_input_lastnum")[0];
const address_input = document.getElementsByClassName("address_input")[0];
const email_input_first = document.getElementsByClassName("email_input_first")[0];
const email_input_second = document.getElementsByClassName("email_input_second")[0];
const email_input_third = document.getElementsByClassName("email_input_third")[0];
const password_input = document.getElementsByClassName("password_input")[0];
const password_check_input = document.getElementsByClassName("password_check_input")[0];
...
const cbx_personal_information_agree = document.getElementsByClassName("cbx_personal_information_agree")[0];
const cbx_third_party_agree = document.getElementsByClassName("cbx_third_party_agree")[0];
...
const inputConfirm = () => {
    ...
    try{
        ...
        phoneNumberRegex.test(phone_number_input_lastnum.value) ? 0 : rtnMsg += " 전화번호 ";
        // 주소 확인
        address_input.value ? 0 : rtnMsg += " 주소 ";
        // 이메일 확인
        emailRegex.test(email_input_first.value) && 
        emailRegex.test(email_input_second.value) && 
        emailRegex.test(email_input_third.value) ? 0 : rtnMsg += " 이메일 "
        // 비밀번호 확인
        passwordRegex.test(password_input.value) ? 0 : rtnMsg += " 비밀번호 ";
        // 비밀번호확인을 확인
        password_check_input.value === password_input.value ? 0 : rtnMsg += " 비밀번호확인 ";
        // 모두 동의 체크 확인
        cbx_personal_information_agree.checked && cbx_third_party_agree.checked ? 0 : rtnMsg += "필수동의"
...
join_button.addEventListener("click", async () => {
    ...
    if(inputConfirm() === true){
        try {
            // 데이터 전달
            //http://kdt-sw-6-team10.elicecoding.com/users/signup
            //https://jsonplaceholder.typicode.com/posts
            await fetch("http://kdt-sw-6-team10.elicecoding.com/api/users/signup", {
                method: "POST",
                headers: {
