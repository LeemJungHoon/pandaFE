...
const allPresentContainer = document.getElementsByClassName("all_present_container")[0];
...
const catagoryDetailBtn = document.getElementsByClassName('catagory_detail_btn')[0];
...
window.addEventListener('DOMContentLoaded',  () => {
  ...
  try {
      ...
      // 쿼리스트링 page=1로 불러서 1페이지에 들어갈 상품값을 불러옴.
        fetch("http://kdt-sw-6-team10.elicecoding.com/api/items/category/Present/?page=1", {
          headers: {
...
searchImage.addEventListener("click",  () => {
  ...
  try {
    // 검색한 데이터 전달
      fetch(`http://kdt-sw-6-team10.elicecoding.com/api/items/search/?name=${searchInput.value}`, {
        headers: {
          "Content-Type": "application/json",
        },
      })
...
totalPageBtn.addEventListener("click",   (e) => {
  ...
  try {
    ...
    let pageUrl = `http://kdt-sw-6-team10.elicecoding.com/api/items/category/Present/?page=${current_page_num}`;
    ...
    const subPageUrl = `http://kdt-sw-6-team10.elicecoding.com/api/items/subCategory/${subCatagory}/?page=${current_page_num}`;
...
catagoryDetailBtn.addEventListener("click",  (e) => {
  ...
  localStorage.setItem("subCatagory", catagoryDetailName);
   ...
   fetch(`http://kdt-sw-6-team10.elicecoding.com/api/items/subCategory/${catagoryDetailName}/`, {
    headers: {
      "Content-Type": "application/json",
    },
  })
