...
const allVaseContainer = document.getElementsByClassName("all_vase_container")[0];
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
      ...
      // 쿼리스트링 page=1로 불러서 1페이지에 들어갈 상품값을 불러옴.
       await fetch("http://kdt-sw-6-team10.elicecoding.com/api/items/category/Vase/?page=1", {
          headers: {
...
searchImage.addEventListener("click", async () => {
  ...
  //  ex) 아니면 Vase에 있는 총 물품의 2페이지인지를 확인하기 위해 사용
  localStorage.removeItem("searchProduct");
  ...
  try {
    // 검색한 데이터 전달
     await fetch(`http://kdt-sw-6-team10.elicecoding.com/api/items/search/?name=${searchInput.value}`, {
        headers: {
          "Content-Type": "application/json",
        },
      })
...
totalPageBtn.addEventListener("click",  async (e) => {
  ...
  try {
    ...
    const searchPageUrl = `http://kdt-sw-6-team10.elicecoding.com/api/items/search/?name=${searchProduct}&page=${current_page_num}`;
    const normalPageUrl = `http://kdt-sw-6-team10.elicecoding.com/api/items/category/Vase/?page=${current_page_num}`;
...
const makeVaseProduct = (data) =>{
  try{
    // 페이지네이션할때 이전에 있었던 이미지는 초기화시켜주고
   allVaseContainer.innerHTML = '';
    ...
    for (
      ...
      const makeVaseProductEl = `
      <a href="/pages/flower_market_detail/${data[i-1].item_id}">
        <div class="vase_container">
          <div class="image"><img src="http://kdt-sw-6-team10.elicecoding.com/${data[i-1].imageUrl}" /></div>
          <span class="vase_name">${data[i-1].name}</span>
          <span class="vase_price">${data[i-1].price}</span>
        </div>
      </a>
    `;
    allVaseContainer.innerHTML += makeVaseProductEl;
    }
  }catch(err){
    console.log("makeVaseProduct 에러 :", err);
  }
};